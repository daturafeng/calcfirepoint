...
The migrated Cesium engine has no standalone hover-coordinate presentation adapter; Cesium's existing `ScreenSpaceEventHandler` remains the correct source for the pointer position. The dock `RealtimeCesiumMap` already solves this interaction class: it pauses hover work for left/middle/right drag and pinch camera interactions, retains the latest pointer position, then resolves it once when the interaction ends. `MapCanvas` follows that established model, performing coordinate resolution with `scene.globe.pick(camera.getPickRay(...))` and falling back to the ellipsoid instead of issuing raw-event `scene.pickPosition` depth reads. It publishes the result at a short bounded interval and cancels scheduled work during viewer cleanup. This prevents depth-picking from blocking camera rotation while preserving Cesium camera controls, map annotation behaviour, and the latest displayed coordinate.

The dock Cesium engine owns camera-height normalization through `cameraHeight.ts` and its `CesiumMapEngine.getCameraHeight()` contract. That module and contract are synchronized into the migrated engine. The standalone map currently owns its specialised Viewer because it loads this project's configured imagery, terrain, and annotation data sources directly; replacing that lifecycle with the dock scene-resource controller would change resource behaviour outside this requirement. The copied full `mapEngine` also retains dock cloud-service imports that are not part of this standalone build, so `MapCanvas` calls the lightweight `cameraHeight` engine module directly rather than importing the complete map-engine factory. Both paths use the same normalization, while `CesiumMapEngine` exposes the matching instance method for consumers that already use the full engine.

### Isolate a pure Cesium engine from dock-only concerns

The copied engine contains dock-only runtime-scene retrieval, authentication-key lookup, layer preference persistence, regional map-center configuration, route-editor domain types, a third-party navigation wrapper, and DJI TerraGS-specific loading. None is required by the fire-point map, and the cloud-scene modules prevent the standalone engine from being compiled or imported independently. The pure boundary retains standard Cesium Viewer, imagery provider, standard 3D Tiles, draw, annotation, camera, coordinate, and height utilities. Resource URLs and optional load-error callbacks are supplied by callers.

The dock scene-resource controller and related cloud-layer API are removed rather than replaced by no-op behaviour: a no-op would preserve a misleading API that appears to load resources while silently doing nothing. Dock regional constants, route-editor modules, Umi route adapter, and UAV-model curve helper are removed because they are product-domain concerns, not generic map-engine capabilities. The TerraGS branch and third-party navigation wrapper are likewise excluded from this first pure engine boundary; callers still have standard Cesium camera and toolbar primitives. If a product later needs these optional integrations, it must provide them outside the engine through an explicit adapter with its own dependencies.
